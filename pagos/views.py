...
class PagoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        registrar_auditoria(
            self.request, "CREACIÓN",
            f"Se registró pago de ${instance.valor_pago} para matrícula {instance.matricula} (ID: {instance.pk})"
        )

    def perform_update(self, serializer):
        instance = serializer.save()
        registrar_auditoria(
            self.request, "ACTUALIZACIÓN",
            f"Se actualizó el pago ID {instance.pk} - Estado: {instance.estado_pago}"
        )
